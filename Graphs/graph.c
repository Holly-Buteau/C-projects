...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	Deque *newD = dequeNew();	
	dequePushBack(newD, source);
	
	while(!dequeIsEmpty(newD))
	{ 
		Vertex *cur = dequeBack(newD);
		dequePopBack(newD);
		cur->isVisited = 1;
		if (cur == destination)
		{
		...
		int x;
		for (x = 0; x < cur->numNeighbors; x++)
		{
			if (!(cur->neighbors[x]->isVisited))
				dequePushBack(newD, cur->neighbors[x]);
		}
	}
